...
const CalendarMonthViewEventsPanel = React.memo<CalendarMonthViewEventsPanelProps>(({ date }) => {
  ...
  return (
    <React.Fragment>
      {firtsEvents.map((event) => (
        <CalendarEventCard key={event.id} event={event} resizeOrientation="horizontal">
...
const CalendarMonthDayButton = React.memo<CalendarMonthDayButtonProps>(({ date }) => {
  const calendar = useCalendar();
  const formatters = calendar.getFormatters();

  const isCurrentDay = React.useMemo(() => isSameDay(date, new Date()), [date]);
...
const CalendarMonthDay: React.FC<CalendarMonthDayProps> = React.memo(({ date, isOutside }) => {
  return (
    <div
      className={cn(
