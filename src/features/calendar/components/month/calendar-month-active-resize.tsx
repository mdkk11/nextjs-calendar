...
import { DateFragmentType, useDateFragments } from '@/features/calendar/hooks';

import React from 'react';

import { useCalendar } from '@/features/calendar/components/calendar';
import { useMonthViewPosition } from '@/features/calendar/hooks/month/use-month-view-position';
...
const CalendarMonthActiveResizeContent = React.memo<CalendarMonthActiveResizeContentProps>(
  ({ date, resizingEvent, fragmentType }) => {
    const position = useMonthViewPosition(date);

    return (
      <div className="absolute py-1" style={{ ...position, height: '2.25rem' }}>
...
CalendarMonthActiveResizeContent.displayName = 'CalendarMonthActiveResizeContent';
