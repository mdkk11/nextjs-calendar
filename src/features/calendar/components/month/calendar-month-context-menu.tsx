...
interface CalendarMonthContextMenuProps extends React.HTMLAttributes<HTMLDivElement> {
  children: React.ReactNode;
}

const CalendarMonthContextMenu: React.FC<CalendarMonthContextMenuProps> = ({ children }) => {
      ...
      <ContextMenuContent className="w-64" container={ref.current!}>
        {activateEvent ? (
          <React.Fragment>
              ...
              >
                {CALENDAR_COLORS.map((color) => (
                  <div key={color} className="flex items-center space-y-0">
                    <RadioGroupItem id={color} value={color} className="hidden" />
                    <label
                      htmlFor={color}
                      className={cn(
            ...
            <ContextMenuItem onSelect={() => calendar.cutEvent(activateEvent.id)}>
