...
import { useCalendarMonthDrag } from '@/features/calendar/hooks/month/use-calendar-month-drag';
import { useCalendarMonthInteraction } from '@/features/calendar/hooks/month/use-calendar-month-interaction';
import { useCalendarMonthResize } from '@/features/calendar/hooks/month/use-calendar-month-resize';
import { useCalendarMonthSelection } from '@/features/calendar/hooks/month/use-calendar-month-selection';
import { cn, mergeRefs } from '@/libs';
import { createCalendarView } from '@/libs/calendar';
import { addMonths, getMonthDays } from '@/utils/time';
...
const CalendarMonthView = React.forwardRef<HTMLDivElement, CalendarMonthViewProps>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div ref={ref} className={cn('flex h-full flex-grow flex-col', className)} {...props}>
        <CalendarMonthHeader />
        <div className="relative flex h-0 flex-grow flex-col select-none">
          <CalendarMonthContextMenu>
            ...
            >
              ...
              {dates.map((day, index) => {
                return <CalendarMonthDay key={index} date={day.date} isOutside={day.isOutside} />;
              })}
...
const view = createCalendarView<typeof VIEW_ID, CalendarMonthMeta, CalendarMonthConfiguration>({
  id: VIEW_ID,
