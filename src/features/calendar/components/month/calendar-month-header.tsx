...
interface CalendarMonthHeaderProps extends React.HTMLAttributes<HTMLDivElement> {
  className?: string;
}

const CalendarMonthHeader = React.forwardRef<HTMLDivElement, CalendarMonthHeaderProps>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn('grid h-12 w-full flex-none grid-cols-7 items-center border-b', className)}
        {...props}
      >
        {dates.map((_, i) => (
