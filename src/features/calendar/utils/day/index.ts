...
const getRowFromTime = (
  ...
  return Math.floor(
    (date.getHours() - startHour) * rowsPerHour + (date.getMinutes() + 1) / minutesPerRow,
  );
};
...
const computePositionFromTime = (
  ...
  const dateIndex = dates.findIndex((date) => date.date.toDateString() === startAt.toDateString());
...
export { computeEventTimeRangeFromPointer, computePositionFromTime, getDateFromXPosition };
