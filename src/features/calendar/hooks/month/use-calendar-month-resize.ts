...
import { computeEventTimeRangeFromPointer } from '@/features/calendar/utils/month';
    ...
    (s) => {
      ...
      if (!s.resizingEvent) {
        const handleContainerMouseDown = (event: MouseEvent) => {
          const { resizeTopEventId, resizeBottomEventId } = (event.target as HTMLElement).dataset;
      ...
      const handleResize = (event: MouseEvent) => {
        ...
        if (isResizingBottom) {
          const { endAt } = computeEventTimeRangeFromPointer(event, container, calendar);
