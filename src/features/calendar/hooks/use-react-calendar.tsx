...
interface UseCalendarOptions<
  ...
  TViews extends CalendarView<CalendarViewId, CalendarViewMeta, CalendarViewConfiguration>[],
...
>({
  ...
  const calendarRef = React.useRef<CalendarApi<TEvent, TViews>>({
    ...
    getViewMeta: (viewId) => storeRef.current.state.views.find((v) => v.id === viewId)!.meta,
    ...
    increaseDate: () => {
      const newDate = storeRef.current.state.currentView.increaseFn(storeRef.current.state.date);
      calendarRef.current.changeDate(newDate);
    },
    decreaseDate: () => {
      const newDate = storeRef.current.state.currentView.decreaseFn(storeRef.current.state.date);
      calendarRef.current.changeDate(newDate);
    },

    // Event feature
    getEvents: () => storeRef.current.state.events,
    getEvent: (eventId) => storeRef.current.state.events.find((event) => event.id === eventId),
    addEvent: (event) => {
      storeRef.current.update((state) => ({
        ...state,
    ...
    updateEvent: (event) => {
      storeRef.current.update((state) => ({
        ...state,
        events: sortEvents(
          state.events.map((prevEvent) => (prevEvent.id === event.id ? event : prevEvent)),
        ),
      }));
      onEventUpdate?.(event);
    },
    ...
    useDayEvents: (date: Date) => {
      ...
      return useStore(storeRef.current, (s) =>
        s.events.filter((event) => isDatesBetween(startAt, endAt, event.startAt, event.endAt)),
      );
    },
    useViewAnimation: () => {
      return useStoreEffect(
        ...
        (state, previousState) => {
          ...
          if (state.date > previousState.date) {
            view.classList.add('animate-in', 'slide-in-from-right-1/4', 'fade-in', 'duration-300');
          ...
          if (state.date < previousState.date) {
            view.classList.add('animate-in', 'slide-in-from-left-1/4', 'fade-in', 'duration-300');
            const timeout = setTimeout(() => {
              view.classList.remove(
                'animate-in',
    ...
    useViewAutoScroll: () => {
      return useStoreEffect(
        storeRef.current,
        (s) => s.isDragging || s.isResizingTop || s.isResizingBottom || s.isSelecting,
        (state) => {
          const view = storeRef.current.state.viewRef.current;
      ...
      return useStoreEffect(
        ...
        (s) => {
          if (s.activeEvent) {
            ...
            const duplicateEvent = storeRef.current.state.shortcuts.duplicateEvent;
    ...
    pasteEvent: ({ startAt }: { startAt: Date }) => {
      ...
      if (copiedEvent) {
        const duration = copiedEvent.endAt.getTime() - copiedEvent.startAt.getTime();
      ...
      if (cuttedEvent) {
        const duration = cuttedEvent.endAt.getTime() - cuttedEvent.startAt.getTime();
    ...
    openCreationForm: (event: Partial<CalendarEvent<TEvent>>, onClose?: () => void) => {
      const showFunc = storeRef.current.state.formView === 'dialog' ? showDialog : showSheet;
      showFunc({
        ...
        render: (close) => (
          <CalendarEventForm
            onSubmit={(values) => {
              ...
              toast(storeRef.current.state.translations.message['event-created']);
              close();
            }}
            defaultValues={{
        ...
        render: (close) => (
          <CalendarEventForm
            onSubmit={(values) => {
              ...
              toast(storeRef.current.state.translations.message['event-updated']);
              close();
            }}
            defaultValues={{
              summary: event.summary,
